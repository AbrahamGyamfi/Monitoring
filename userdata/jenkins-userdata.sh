...
for i in {1..12}; do
    APP_SERVER_IP=$(aws ec2 describe-instances --region $REGION --filters "Name=tag:Name,Values=taskflow-app" "Name=instance-state-name,Values=running" --query 'Reservations[0].Instances[0].PublicIpAddress' --output text 2>/dev/null)
...
APP_PRIVATE_IP=$(aws ec2 describe-instances --region $REGION --filters "Name=tag:Name,Values=taskflow-app" "Name=instance-state-name,Values=running" --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text 2>/dev/null || echo "")
